...
//
// For usage totals in the stream, set req.StreamOptions.IncludeUsage = true.
...
func (s *Stream) Recv() (*ChatCompletionChunk, error) {
	...
	if err := json.Unmarshal(data, &chunk); err != nil {
		s.err = fmt.Errorf("deepseek: decode stream chunk: %w", err)
		return nil, s.err
	}
	s.accumulate(&chunk)
	return &chunk, nil
...
func (s *Stream) readData() ([]byte, error) {
	...
	for {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("deepseek: read stream: %w", err)
		}
...
func (s *Stream) accumulate(chunk *ChatCompletionChunk) {
	...
	for _, cc := range chunk.Choices {
		...
		ch.Message.ReasoningContent += cc.Delta.ReasoningContent
