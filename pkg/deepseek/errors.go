...
func (e *APIError) Error() string {
	...
	return fmt.Sprintf("deepseek: %d %s/%s: %s%s", e.StatusCode, e.Type, e.Code, e.Message, id)
}

// Retryable reports whether the request may be retried. DeepSeek returns 429
// under load and 500/503 for transient server/resource issues.
